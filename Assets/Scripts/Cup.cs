...
public class Cup {
	...
	private CylinderTargetBehaviour cupTargetBehaviour;
	private Vector3[] pointsOnCupLocal;
	...
	public void update ()
	{
		...
		foreach (var pt in pointsOnCupLocal) {
			pointsOnWorld.Add (cupTargetBehaviour.transform.TransformPoint (pt));
		}
	...
	{
		return cupTargetBehaviour.CurrentStatus == TrackableBehaviour.Status.NO_POSE;
	}

	public bool isTracked ()
	{
		return cupTargetBehaviour.CurrentStatus == TrackableBehaviour.Status.TRACKED;
	}

	// カメラを太陽に見立てて、カップ上面とカメラの南中高度. 0度->0, 90度->1.
	public double culminationAltitude ()
	{
		return ellipseRotatedRect.size.width / ellipseRotatedRect.size.height;
	}

	// カップ上面の点群の外接矩形を取得. OpenCVの画像座標系
	public OpenCVForUnity.Rect getTopSurfaceRect ()
	{
		return Imgproc.boundingRect (new MatOfPoint (pointsOnCvScreen.ToArray ()));
	}

	// カップ上面の点群の外接矩形を取得. OpenCVの画像座標系. ただし、ROIが画像外の座標になることもあるので、ROIの最小・最大を画像領域内におさめる
